...
const tkgmClient = new TKGMClient({
  ...
  userAgent: 'Kadastro360-Web-Pilot/1.9.4'
});
...
function marketingPage({ loginMessage = '', requestMessage = '', requestOk = false, user = null } = {}) {
  ...
  <style>
    ...
    *{box-sizing:border-box}html{scroll-behavior:smooth}body{margin:0;background:linear-gradient(180deg,#eef3f6 0%,#f8fbfc 100%);color:var(--text);font-family:Arial,sans-serif}
    ...
    <div class="topbar">
      <div><img class="brand-logo-main" src="/assets/kadastro360-logo-horizontal.png?v=20260723-2" alt="Kadastro360"><div class="brand-sub">Parsel sorgusu, eğim analizi, yakın yerler ve açık kamu katmanları tek ekranda.</div></div>
    ...
    <div class="hero">
      ...
      <aside class="card auth" id="giris">
        <img class="auth-logo" src="/assets/kadastro360-logo-horizontal.png?v=20260723-2" alt="Kadastro360">
        <h2>Hızlı erişim</h2>
      ...
      for (const snapRadius of [null, 1800]) {
        ...
        try {
          const payload = await fetchJson(url.toString(), {
            headers: { 'User-Agent': 'Kadastro360-Web-Pilot/1.9.4', Accept: 'application/json' }
          }, 9000);
  ...
  return cached(key, 7 * 24 * 60 * 60 * 1000, async () => {
    ...
    const rows = await fetchJson(url.toString(), {
      headers: {
        Accept: 'application/json',
        'Accept-Language': 'tr-TR,tr;q=0.9',
        'User-Agent': 'Kadastro360-Web-Pilot/1.9.4 (kadastro360.com.tr)'
      }
    }, 9000);
