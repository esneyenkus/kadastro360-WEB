...
const WMS_CONFIGS = [
  ...
  },
  {
    ...
    tileRequestSize: 512,
    maxUsefulZoom: 14,
    choiceRevision: 'ergene-kayseri-tile-1'
  },
  {
    key: 'cdp-kirikkale',
    provinces: ['Kırıkkale'],
...
const STATIC_REGION_LINKS = [
  {
    provinces: ['Balıkesir'], districts: ['Gönen'],
    title: 'Balıkesir–Çanakkale Çevre Düzeni Planı',
    description: 'Gönen’i kapsayan yürürlükteki resmî 1/100.000 çevre düzeni planı kaynak sayfasını açar. Kamuya açık ve parsel koordinatında doğrulanmış canlı WMS bulunmadığı için haritaya tahmini katman eklenmez.',
    category: 'plan-source',
    sourceUrl: 'https://mpgm.csb.gov.tr/balikesir-canakkale-planlama-bolgesi-1-100-000-olcekli-cevre-duzeni-plani-82273'
  },
  {
    provinces: ['Kayseri'],
...
async function fetchBuffer(url, options = {}, timeoutMs = 10000, maxBytes = 8_000_000) {
  ...
  try {
    const response = await fetch(url, {
      ...
      headers: {
        'User-Agent': 'Kadastro360/2.0.6 (+viewport-wms)',
        Accept: '*/*',
        ...(options.headers || {})
      }
    });
...
function directWmsDefinition(config, resolved = null) {
  ...
  return {
    ...
    loadMode: 'single-viewport-resolution'
  };
}
...
async function buildPilotCatalog({ province, district, detailed = false }) {
  ...
  return {
    ...
    wmsLoadMode: 'single-viewport-resolution',
    supportedRegions: [
