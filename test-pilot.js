...
(async () => {
  ...
  assert(html.includes('single-viewport-resolution') && html.includes('projectedWmsView(item,store.map.getBounds())'), 'Tek görünür alan çözünürlüklü WMS modu eksik.');
  assert(openDataCode.includes("loadMode: 'single-viewport-resolution'"), 'Görünür alan WMS yükleme modu eksik.');
  ...
  assert(ysk?.wms?.loadMode === 'single-viewport-resolution', 'WMS tek görünür alan modunda değil.');
  ...
  console.log('Kadastro360 v2.0.8 çalışan WMS korunumu, sunucu ve tarayıcı yakın yer yedeği ile kurumsal özellikler doğrulaması geçti.');
})().catch(error => {
  console.error(error);
  process.exitCode = 1;
});
